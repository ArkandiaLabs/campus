...
export default function OfferingCard({ offering }: OfferingCardProps) {
  const progressPercent =
    offering.total_contents > 0
      ? Math.round((offering.completed_contents / offering.total_contents) * 100)
      : 0;
  ...
  return (
    ...
    >
      ...
      {dateRange && <p className="text-sm text-gray-500 mb-3">{dateRange}</p>}

      <div className="mt-3">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>
            {offering.completed_contents} de {offering.total_contents} completados
          </span>
